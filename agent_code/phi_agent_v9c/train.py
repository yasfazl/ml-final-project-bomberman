...
REWARD_REVISITED_POSITION     = -6.0
REWARD_GOOD_BOMB              = 10.0
REWARD_USELESS_BOMB           = -6.0
REWARD_SUICIDE_BOMB           = -10.0

# Opponent-related. Neither can fire in solo play.
#
# Measured, not assumed: environment.py:262 adds GOT_KILLED to every agent in
# agents_hit, and an agent that blew itself up is in that set. A probe agent that
# bombed its own tile ended the round with
#   [... 'KILLED_SELF', 'GOT_KILLED']
# so with the values below a suicide now costs KILLED_SELF (-20) plus
# GOT_KILLED (-20) = -40, where the solo agent was tuned against -20. Set
# GOT_KILLED_EXCLUDES_SUICIDE = True to charge only KILLED_SELF for suicides and
# leave GOT_KILLED for deaths caused by someone else.
REWARD_KILLED_OPPONENT        = 20.0
REWARD_GOT_KILLED             = -20.0

GOT_KILLED_EXCLUDES_SUICIDE   = False
...
def game_events_occurred(
    ...
    if summary_old is not None and summary_new is not None:
        ...
        if not DISABLE_BOMB and self_action == "BOMB":
            ...
            if summary_old["trap"]:
                ...
                summary_old["crate_efficiency"] > 0.0
                or summary_old["enemy_in_blast"] > 0.0
            ):
    ...
    #
    # events.count(), not "in": one bomb can raise CRATE_DESTROYED several
    # times in a single step (once per crate) and can raise KILLED_OPPONENT
    # more than once. Testing membership counted each such step as exactly 1
    # and undercounted the quantity being compared.
    ...
    self.killed_self += events.count(e.KILLED_SELF)
    self.invalid_actions += events.count(e.INVALID_ACTION)
    self.kills += events.count(e.KILLED_OPPONENT)
    self.killed_by_opponent += _deaths_by_opponent(events)
...
def _deaths_by_opponent(events: List[str]) -> int:
    """
    1 if this step killed us and someone else did it, else 0.

    environment.py:261 adds GOT_KILLED to every agent in agents_hit, and an
    agent that blew itself up is in that set, so GOT_KILLED on its own does not
    mean "an opponent killed me". Subtracting the suicides is what separates the
    two, and the framework records neither: EVENT_STAT_MAP in agents.py maps
    KILLED_OPPONENT and KILLED_SELF but not GOT_KILLED, so --save-stats cannot
    report this number at all. Counting it here keeps it inside agent_code,
    which is the only directory that survives submission.

    Ambiguous case, documented rather than guessed at: if our own bomb and an
    opponent's reach us on the same step, both events fire and this returns 0.
    """
    if e.GOT_KILLED in events and e.KILLED_SELF not in events:
        return 1
    return 0
...
def reward_from_events(self, events: List[str]) -> float:
    ...
    counted = events
    if GOT_KILLED_EXCLUDES_SUICIDE and e.KILLED_SELF in events:
        counted = [event for event in events if event != e.GOT_KILLED]

    total = sum(reward_map.get(event, 0.0) for event in counted)
