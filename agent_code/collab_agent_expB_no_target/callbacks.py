...
def valid_action_indices(game_state: dict) -> list[int]:
    ...
    if bool(game_state["self"][2]):
        # Gate experiment: the has_target requirement is lifted here.
        #
        # Measured on 40 traced rounds, BOMB reached the candidate set in only
        # 7.0% of steps while a bomb was actually available in about 76% of
        # them, and the learned policy accepted BOMB in 68% of the steps where
        # it was offered.  The bomb rate was therefore set by this gate, not by
        # the Q-values.  opus_agent_task2 destroys the same 29 crates per round
        # from 41 bombs that this agent gets from 15.5, and its extra bombs --
        # the ones that hit nothing at drop time -- are where its higher kill
        # count comes from.  Both escape checks are deliberately kept, so the
        # survival guarantee that keeps the suicide rate at 0.048 is untouched.
        if has_escape_route_after_bomb(game_state):
