# Hunt fine-tuning: direction and proximity to the nearest opponent, plus the
# movement reward that was missing for them, both active only once the crates
# are gone.  Previous stage: local one-step crate-efficiency fine-tuning (v2).
...
# Weight columns that receive gradient updates in this run: the five hunt
# columns and nothing else.  Everything the agent already does well -- coin
# collection, crate bombing, escaping -- keeps the weights it was trained
# with, so any behavioural change can only come from the new block.
#
# Set to None to update every column.
TRAINABLE_COLUMNS = (39, 40, 41, 42, 43)
...
# Walking towards an opponent.  Every other objective already had a per-step
# gradient -- coins +1.0, crates +0.75, safety +1.5 -- and opponents had none,
# which is the most likely reason two separate agents both learned to keep
# away rather than close in: the weight on "a bomb here would hit an opponent"
# stayed at exactly zero here, and the other model learned -3.8 and -5.0 on
# "an enemy lies ahead" in two independent training runs.
#
# Both events fire only while no crate is left and no coin is reachable, the
# same gate as features 39-43, so the reward and the state representation can
# never disagree about when hunting is the thing to do.
#
# The penalty is the larger of the two, as the project brief asks, so that
# stepping back and forth beside an opponent cannot farm the bonus.
REWARD_MOVED_TOWARD_OPPONENT = 1.0
REWARD_MOVED_AWAY_FROM_OPPONENT = -1.5
...
def setup_training(self):
    ...
    # are restricted to the columns named in TRAINABLE_COLUMNS.
    if TRAINABLE_COLUMNS is None:
        self.trainable_mask = None
    else:
        self.trainable_mask = np.zeros(
            self.model.shape[1],
            dtype=np.float64,
        )
        self.trainable_mask[list(TRAINABLE_COLUMNS)] = 1.0
    ...
    self.logger.info(
        f"TD Q-learning hunt fine-tuning initialized: "
        f"episodes={self.episodes_trained}, "
        f"epsilon={self.epsilon:.3f}, "
        f"trainable_columns={TRAINABLE_COLUMNS}"
    )
...
def game_events_occurred(
    ...
    add_opponent_distance_event(old_game_state, new_game_state, events)
...
def update_q_learning(
    ...
    trainable_mask = getattr(self, "trainable_mask", None)

    if trainable_mask is None:
        update = ALPHA * td_error * features
    else:
        update = ALPHA * td_error * features * trainable_mask

    self.model[action_index] += update
...
def add_opponent_distance_event(
    old_game_state: dict,
    new_game_state: dict,
    events: List[str],
):
    """Add an event for progress toward the nearest opponent.

    Gated exactly like features 39-43: no crate left anywhere on the field and
    no coin the agent could still walk to.  While either remains, the agent
    keeps doing what it is already good at and this event never fires.
    """
    if old_game_state is None or new_game_state is None:
        return

    if np.any(old_game_state["field"] == 1):
        return

    if nearest_coin_path(old_game_state)[0] is not None:
        return

    old_distance = nearest_opponent_path(old_game_state)[1]
    new_distance = nearest_opponent_path(new_game_state)[1]
...
def save_model(self):
    ...
    saved_data = {
        ...
        "training_stage": "hunt_features_finetuning",
    }
