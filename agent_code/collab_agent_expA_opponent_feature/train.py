# Opponent-feature fine-tuning: revive feature 20 by training with opponents.
# Previous stage: local one-step crate-efficiency fine-tuning (version 2).
...
# Weight columns that receive gradient updates in this run.
#
# The previous stage (local_crate_efficiency_fine_tuning_v2) froze columns
# 0-31 and trained only 32-38.  Feature 20 ("a bomb placed here would hit an
# opponent") sits at index 20, inside that frozen block, so it kept its
# initial 0.0 through all 3701 solo episodes -- the six weights in that column
# are still exactly zero.  This run unfreezes that single column and nothing
# else, so any behavioural change is attributable to w[:, 20] alone.
#
# Set to None to update every column (the original unfrozen behaviour).
TRAINABLE_COLUMNS = (20,)
...
def save_model(self):
    ...
    saved_data = {
        ...
        "training_stage": "opponent_feature_20_finetuning",
    }
