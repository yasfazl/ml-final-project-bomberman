...
# 11:   danger_countdown: 0=safe (no bombs), 1-4=steps until nearby bomb explodes
#       Set to 0 when DISABLE_BOMB=True to avoid constant-feature bias
...
# 17:   can_escape: 1 if agent can reach a safe tile within BOMB_TIMER steps
#       after placing a bomb at current position, else 0
...
def state_to_features(game_state: dict) -> np.ndarray | None:
    ...
    # When DISABLE_BOMB=True (coin-heaven), bombs never exist so this would
    # always return 4.0 — a constant that acts as a second bias term.
    # We set it to 0.0 in that case to avoid collinearity with feature 0.
    if DISABLE_BOMB:
        features[11] = 0.0
    else:
    ...
    # Features 17-21: escape planning
    # At every step, compute the escape direction based on currently active
    # bombs (hypothetical=False). This keeps the feature valid throughout
    # an escape sequence, not just at the moment of placing a bomb.
    if DISABLE_BOMB:
        # No bombs in coin-heaven; escape features are meaningless.
        features[17] = 0.0
    else:
        escape_dir, can_escape = escape_path(x, y, field, bombs, hypothetical=False)
        features[17] = float(can_escape)
...
def danger_countdown(
    ...
    """
    Return the minimum countdown among bombs threatening (x, y), or 4.0 if safe.
    """
    min_countdown = 4
