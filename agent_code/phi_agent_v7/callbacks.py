...
#  33        an escape route exists
#  34        adjacent ongoing explosions / 4
#
# BOMB block
#  35        bomb bias
#  36        number of legal moves / 4
#  37        explosion_efficiency
#  38        bomb here would trap us
#  39        a coin is visible
MOVE_OFFSET, MOVE_DIM = 0, 27
WAIT_OFFSET, WAIT_DIM = 27, 8
BOMB_OFFSET, BOMB_DIM = 35, 5

FEATURE_DIM = MOVE_DIM + WAIT_DIM + BOMB_DIM   # 40
...
def escape_path(
    ...
    A tile holding a live bomb is impassable, exactly as in
    ...
    bomb_tiles = {position for position, _timer in bombs}
    ...
    while queue:
        ...
        for direction_index, action in enumerate(MOVE_ACTIONS):
            ...
            if not (0 <= nx < field.shape[0] and 0 <= ny < field.shape[1]):
                continue
            if field[nx, ny] != 0 or next_pos in bomb_tiles:
                continue
