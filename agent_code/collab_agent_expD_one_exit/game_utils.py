...
# Movement budget for the two-exit search below.
#
# Measured on 381 states where no crate was left on the field and an opponent
# stood in the blast of a bomb the agent could have dropped:
#
#     2 steps / 2 exits (the original)     2.4% admitted
#     2 steps / 1 exit                     3.1%
#     3 steps / 2 exits                   38.1%
#     3 steps / 1 exit                   100.0%
#     4 steps / 2 exits                   99.5%
#
# The ordinary escape check admitted 100% of the same states, so with a budget
# of two this gate was refusing almost every chance the agent had to attack.
# Loosening the exit count barely moved it (2.4% -> 3.1%); the budget is what
# binds.  Three steps with two exits keeps the original idea -- one opponent
# cannot stand on both exits -- while admitting sixteen times as many
# opportunities, and it still turns down the majority.
#
# The wider budget applies only once every crate is gone.  While crates remain
# the agent is already doing well (1.88 crates per bomb, a suicide rate of
# 0.048) and none of that behaviour should change.
ESCAPE_STEPS_WITH_CRATES = 2
ESCAPE_STEPS_NO_CRATES = 3

# How many distinct exits the search has to find, again split by whether any
# crate is left.
#
# Two exits guard against an opponent stepping into the one escape square and
# turning the bomb into a suicide.  That is worth paying for while crates are
# still standing, because dying then costs every coin the agent has not
# collected yet.  Once the crates and coins are gone it is not: measured over
# 200 rounds on one seed, going from the original gate to three steps and two
# exits raised the agent's own suicides from 12 to 62 but *dropped* the times
# an opponent killed it from 38 to 11, because an agent that is already dead
# cannot be killed for someone else's five points.  Its kill balance went from
# -120 to +115 points.  Requiring only one exit after the crates are gone
# takes that trade further, at the risk that dying even earlier costs the
# agent the kills it would have made later.
EXITS_WITH_CRATES = 2
EXITS_NO_CRATES = 1
...
def bomb_has_robust_escape_route(
    game_state: dict,
    maximum_escape_steps: int | None = None,
    required_exits: int | None = None,
) -> bool:
    ...
    opponent is far away.  Otherwise this gate requires ``required_exits``
    distinct exit tiles outside the engine-accurate blast within
    ``maximum_escape_steps`` movements.  One opponent cannot occupy both
    exits at once.

    ``maximum_escape_steps`` defaults to the crate-dependent budget described
    above; pass an explicit value to override it.
    """
    ...
    crates_left = bool(np.any(field == 1))

    if maximum_escape_steps is None:
        maximum_escape_steps = (
            ESCAPE_STEPS_WITH_CRATES
            if crates_left
            else ESCAPE_STEPS_NO_CRATES
        )

    if required_exits is None:
        required_exits = (
            EXITS_WITH_CRATES
            if crates_left
            else EXITS_NO_CRATES
        )
    ...
    for movement_step in range(1, maximum_escape_steps + 1):
        ...
        if len(safe_exit_positions) >= required_exits:
            return True
    ...
    return len(safe_exit_positions) >= required_exits
