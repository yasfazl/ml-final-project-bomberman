...
#  13 - 16   escape route starts here   [ahead, right, behind, left]
#  17        a coin is visible
#  18        normalised distance to nearest coin
#  19        danger_countdown of the tile we stand on
#  20        explosion_efficiency of the tile we stand on
#  21        dropping a bomb here would trap us
#  22        danger x escape-ahead        (interaction)
#  23        danger x explosion-ahead     (interaction)
#  24        coin distance x target-ahead (interaction)
#
...
MOVE_OFFSET, MOVE_DIM = 0, 25
WAIT_OFFSET, WAIT_DIM = 25, 9
BOMB_OFFSET, BOMB_DIM = 34, 7

FEATURE_DIM = MOVE_DIM + WAIT_DIM + BOMB_DIM   # 41

# Interaction features 22-24. Set to False to ablate them; the vector keeps its
...
def phi(summary: dict, action_index: int) -> np.ndarray:
    ...
    if action_index < 4:
        ...
        vector[13:17]  = escape
        vector[17]     = summary["coin_visible"]
        vector[18]     = summary["target_distance"]
        vector[19]     = summary["danger"]
        vector[20]     = summary["crate_efficiency"]
        vector[21]     = summary["trap"]
        ...
        if USE_INTERACTIONS:
            ...
            vector[22] = summary["danger"] * escape[0]
            vector[23] = summary["danger"] * explosion[0]
            vector[24] = summary["target_distance"] * target[0]
